    ...
    <x-auth-card>
        ...
        <form method="POST" action="{{ route('permissions.update', $permission->id) }}">
            ...
            <div>
                ...
                <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value=" old('name') ?? $permission->name ?? 'default' " required
                    autofocus />
            </div>
            ...
            <div>
                ...
                @if($permission->roles->contains($role->id)) checked=checked @endif>
                {{$role->name}}
                <br/>
                @endforeach
            </div>
